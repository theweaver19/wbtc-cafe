...
export async function signInOrRegister(
  ...
  try {
    ...
    //FIXME: we should probably handle wrong signatures here, as it would imply some sort of corruption or attack
    console.error(e);
  }
  const { user } = (await firebase.auth().createUserWithEmailAndPassword(id, signature))
  if (!user) return null;
